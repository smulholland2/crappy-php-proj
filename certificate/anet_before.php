...
$cert_month = $_POST['cert_month'];
$cert_day = $_POST['cert_day'];
$cert_year = $_POST['cert_year'];
$cert_email = $_POST['cert_email'];
$UU = $_POST['UU'];
$a2 = $_POST['a2'];
$last4 = $_POST['last4'];
...
$x_relay_URL = 'http://www.tapseries.com/certificate/receipt_before.php';
$ID = $x_invoice_num."|".$last4;
$Description = $_POST['Description'];

$DOB = $cert_month."/".$cert_day."/".$cert_year;

// enters billing information and creates the actual invoice
$SQL = " INSERT INTO anchorage_invoices (UN, TOTAL, PAY, FN, LN, AA1, AA2, ACI, AST, AZ, ACO, AP, AM, NCPY,MARCH1, CERT_FN, CERT_LN, CERT_DOB, CERT_EMAIL)
          VALUES ('$UU', '$amount', 'incomplete', ".mssql_escape($x_first_name).", ".mssql_escape($x_last_name).", ".mssql_escape($x_address).", ".mssql_escape($a2).", ".mssql_escape($x_city).", ".mssql_escape($x_state).", ".mssql_escape($x_zip).", ".mssql_escape($x_country).", '$x_phone', ".mssql_escape($x_email).", ".mssql_escape($x_company).", 'before', ".mssql_escape($cert_fn).", ".mssql_escape($cert_ln).", '$DOB', ".mssql_escape($cert_email).")
           ";
