...
24 => "akan", //Anchorage, Alaska
25 => "WVRN" //Randolph-Elkins County, WV
);
...
//completions
if ($ME==19 || $ME==3 || $ME==17 || $ME==22  || $ME==24 || $v=="WVRN") 
{ 
...
while ($row = mssql_fetch_array($resultset0)) 
	{
		...
		if(strtotime($DA)>strtotime($DE)){
			$DA=date_create($DA);
			date_sub($DA,date_interval_create_from_date_string("8 hours"));	
			$DA = date_format($DA,"m/d/y");
		}
		else{
			$DA = strtotime($DA);
			$DA = date('m/d/y',$DA);
...
if ($v=="WVRN") 
{
	$SQL2="Select * from [07O2],[07O4] where AN='$UA' and PC='WVRN' and [07O2].OID=[07O4].OID and pay='credit'";
	$resultset2=mssql_query($SQL2, $con);
	while ($row = mssql_fetch_array($resultset2)) 
	{
	    $pri = $row['PRI'];
