...
//get the first 2 letters of the password (voucher)
$password_beginning = substr($password, 0, 2);
$password_beginning = strtoupper($password_beginning);

//if voucher doesn't start with AA, show error message and go back to previous pagessss
if($password_beginning != "AA"){
...
//check if the username exists on 09D
$SQL = "SELECT UU FROM [09D] WHERE UU=".mssql_escape($username)." ";
...
// insert the student information in the databse 09D, 09S, 01C
$SQL96 = " INSERT INTO [09D] (UU, UA, UC, NF, NL, ES, FIN, DA, UM) 
           VALUES (".mssql_escape($username).", ".mssql_escape($classcode).", ".mssql_escape($password).", ".mssql_escape($firstname).", ".mssql_escape($lastname).", ".mssql_escape($lang).", '0', '$todaytime', ".mssql_escape($email).") ";
$resultset96=mssql_query($SQL96, $con);

$SQL95 = " INSERT INTO [09S] (UU) VALUES (".mssql_escape($username).") ";
$resultset95=mssql_query($SQL95, $con);

$SQL92 = " INSERT INTO [AllergenDob] (UU, BD) VALUES (".mssql_escape($username).", '$DOB') ";
...
$SQL8 = " UPDATE [SerialNumber]
		  SET ACTIVATED=1, ACCOUNT_NAME='$classcode', DATE_ACTIVATED='$todaytime', STUDENT_USER_NAME='$username', PRODUCT_ID=166
...
header("Location: congratulationsaa.php?username=$username");
?>